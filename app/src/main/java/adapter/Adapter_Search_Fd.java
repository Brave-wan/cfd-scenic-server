 ...
 */
public class Adapter_Search_Fd extends BaseExpandableListAdapter {
    List<List<SelectOrderFdBean.DataBean.OrderListBean>> list_fd;
    Context mContext;

    public Adapter_Search_Fd(List<List<SelectOrderFdBean.DataBean.OrderListBean>> list, Context mContext) {
        this.list_fd = list;
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        cyzMode.tv_zong.setText("总额：￥" + list_fd.get(groupPosition).get(childPosition).getReal_price());
                    ...
                    public void onSuccess(ResponseInfo<String> responseInfo) {
                        try {
                            ...
                            if (loginBean.getHeader().getStatus() == 0) {
                                ToastUtil.show(mContext,"销核订单成功");
                                Activity_Search_Fd searchFd= (Activity_Search_Fd) mContext;
                                searchFd.selectOrder();
                            } else
