...
import com.JsonBean.SelectOrderFdBean;
...
import adapter.Adapter_Search_Fd;
 ...
 */
public class Activity_Search_Fd extends Activity implements XExpandableListView.IXListViewListener {
    ...
    Adapter_Search_Fd search_fd;
    List<List<SelectOrderFdBean.DataBean.OrderListBean>> list;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        elListView.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
            ...
            public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
                if (list.get(groupPosition).get(childPosition).getGoods_type() == 0) {//单品
                    Intent intent = new Intent(Activity_Search_Fd.this, Activity_Details_FdDp.class);
                    intent.putExtra("orderCode", list.get(groupPosition).get(childPosition).getOrder_code() + "");
                    startActivity(intent);
                } else if (list.get(groupPosition).get(childPosition).getGoods_type() == 1) {//套餐
                    Intent intent = new Intent(Activity_Search_Fd.this, Activity_Details_FdTc.class);
                    intent.putExtra("orderCode", list.get(groupPosition).get(childPosition).getOrder_code() + "");
                    startActivity(intent);
                }
                return true;
    ...
    public void selectOrder() {
        ...
        http.send(HttpRequest.HttpMethod.GET, URL.selectOrder, params,
                new RequestCallBack<String>() {
                    DialogProgressbar dialogProgressbar=new DialogProgressbar(Activity_Search_Fd.this,R.style.AlertDialogStyle);
                    @Override
                    public void onStart() {
                        super.onStart();
                    ...
                    public void onSuccess(ResponseInfo<String> responseInfo) {
                        ...
                        try {
                            SelectOrderFdBean bean = new Gson().fromJson(responseInfo.result, SelectOrderFdBean.class);
                            ...
                            if (i == 0) {
                                if (bean.getData().getOrderList() != null) {
                                    ...
                                    if (list.size() > 0) {
                                        ...
                                        search_fd = new Adapter_Search_Fd(list, Activity_Search_Fd.this);
                                        elListView.setAdapter(search_fd);

                                        elListView.setGroupIndicator(null);
                                        //全部展开
                                        for (int zk = 0; zk < search_fd.getGroupCount(); zk++) {
